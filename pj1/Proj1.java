    ...
    public static class Map1 extends Mapper<WritableComparable, Text, Text, DoublePair> {
        ...
        @Override
            ...
            throws IOException, InterruptedException {
                ...
                // put each targetWord spot into a linked list
                ...
                // pair a distance value with each word
                ...
                while (matcher.find()) {
                    word = matcher.group();
                    if (size > 0) {
                        distLeft = Math.abs(wordIndex - list.get(targetIndex));
                        if (targetIndex < size - 2) {
                            distRight = Math.abs(list.get(targetIndex+1) - wordIndex);
                        }
                        if (distLeft == 0 && targetIndex < size - 1) {
                            targetIndex++;
                        }
                        ...
                        System.out.println("right distance: " + distRight);
                        System.out.println("left distance: " + distLeft);
                        System.out.println();
                        //System.out.println();
                        //System.out.println("double1 " + pair.getDouble1());
                        //System.out.println("double2 " + pair.getDouble2());
                        //System.out.println();
                        
                        // write the word, and the pair (ONE, f(d)) : (word, (num, sum)) : (TEXT, DOUBLEPAIR)
                    } else {
                    ...
                    if (!(word.toString().equals(targetGram))) {
                        context.write(new Text(word), pair);
                    }
                    // go to next word
                    wordIndex++;
                }
    ...
    public static class Combine1 extends Reducer<Text, DoublePair, Text, DoublePair> {
        ...
        @Override
            public void reduce(Text key, Iterable<DoublePair> values,
                ...
                double totalNumberWith0 = 0;
                //System.out.println("-----------------");
                DoublePair pair = new DoublePair();
                for (DoublePair value : values) {
                    if (value.getDouble2() > 0) {
                        context.write(key,value);
                    } else {
                        totalNumberWith0 += value.getDouble1();
                    }
                }
                pair.setDouble1(totalNumberWith0);
                pair.setDouble2(0);
    ...
    public static class Reduce1 extends Reducer<Text, DoublePair, DoubleWritable, Text> {
        @Override
            public void reduce(Text key, Iterable<DoublePair> values,
                ...
                for (DoublePair value : values) {
                    if (value.getDouble2() > 0) {
                        totalNumber += value.getDouble1();
                        totalWithD += value.getDouble1();
                        sumOfF += value.getDouble2();
                    } else {
                        totalNumber += value.getDouble1();
                    }
                    //System.out.println(value.getDouble1());
                    //System.out.println(value.getDouble2());
                }
                double rate;
                if (sumOfF > 0) {
                    rate = sumOfF * Math.pow(Math.log(sumOfF),3) / totalNumber;
                } else {
                    rate = 0;
                }
                DoubleWritable rateWrite = new DoubleWritable(-rate);
                context.write(rateWrite, key);
            }
    }
